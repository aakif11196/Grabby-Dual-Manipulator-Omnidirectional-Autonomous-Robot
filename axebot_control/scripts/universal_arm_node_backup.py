            ...
            if "\n" in self.serial_buffer:
                ...
                for line in reversed(lines[:-1]):
                    ...
                    if len(vals) == 5:
                        ...
                        grip = self.map_val(vals[4], 0, 4095, -2.35, 2.35) #gripper

                        # 2. Publish Arm Trajectory (Command -> Gazebo)
                        arm_m = JointTrajectory()
                        arm_m.header.stamp = self.get_clock().now().to_msg()
                        # Joint names MUST match 'ros2_controllers.yaml'
                        arm_m.joint_names = [
                        ...
                        # Hardware mein 4 joints hain, Gazebo mein 5 hain (Forearm Roll = 0.0)
                        p.positions = [a1, a2, a3, 0.0, a4] 
                        p.time_from_start.nanosec = int(self.execution_delay * 1e9) # Dynamic delay
                        arm_m.points.append(p)
                        self.arm_pub.publish(arm_m)

                        # 3. Publish Gripper Trajectory
                        g_m = JointTrajectory()
                        g_m.header.stamp = self.get_clock().now().to_msg()
                        g_m.joint_names = [f"{self.prefix}_left_finger"]
                        gp = JointTrajectoryPoint()
                        gp.positions = [grip]
                        gp.time_from_start.nanosec = int(self.execution_delay * 1e9) # Dynamic delay
                        g_m.points.append(gp)
                        self.grip_pub.publish(g_m)
                        break # Process only the latest full line
        except Exception as e: 
            pass # Serial noise ignore karein
